...
# Better mime type detection using actual file signature
...
def classify_with_vision(img_bytes, mime_type, user_tag=None,
    ...
    vehicle_prompt = """Analyze this vehicle image for 3D rigging.
        ...
        },
        {
          ...
          "animations": [
            {"clip": "drive", "property": "rotation_euler", "axis": "x",
             "keyframes": [[0,0.0],[30,3.14],[60,6.28]], "loop": true}
          ]
        },
        {
          "name": "rear_axle",
    ...
    Rules:
    - wheel_colors_rgb: identify the actual tire and hub colors from the image as RGB 0-1
    - front wheels y=0.25, rear wheels y=0.75 — DO NOT set all wheels to y=0.5
    - left wheels x=0.15, right wheels x=0.85
    - all wheels z=0.15 (near bottom)
    - adjust positions to match actual wheel locations visible in the image
    ...
    - front wheels z=0.25, rear wheels z=0.75
    - left wheels x=0.15, right wheels x=0.85
    """


    animal_prompt = f"""Analyze this 3D object for Blender rigging. Return ONLY valid JSON.{tag_context}{bounds_info}{joints_instruction}
    ...
    {{
      ...
      "joint_hints": [
        {{
          "name": "joint_root", "body_part": "torso", "deforms_mesh": false,
          "position_normalized": {{"x": 0.5, "y": 0.0, "z": 0.5}},
          "animations": []
        }},
        {{
          "name": "joint_pelvis", "body_part": "pelvis", "deforms_mesh": false,
          "position_normalized": {{"x": 0.5, "y": 0.20, "z": 0.5}},
          "animations": [
            {{"clip": "walk", "property": "rotation_euler", "axis": "z",
              "keyframes": [[1,0.0],[15,0.05],[30,0.0],[45,-0.05],[60,0.0]], "loop": true}}
          ]
        }},
        {{
          "name": "joint_spine", "body_part": "spine", "deforms_mesh": false,
          "position_normalized": {{"x": 0.5, "y": 0.40, "z": 0.5}},
          "animations": [
            {{"clip": "walk", "property": "rotation_euler", "axis": "z",
              "keyframes": [[1,0.0],[15,0.04],[30,0.0],[45,-0.04],[60,0.0]], "loop": true}}
          ]
        }},
        {{
          "name": "joint_chest", "body_part": "chest", "deforms_mesh": false,
          "position_normalized": {{"x": 0.5, "y": 0.55, "z": 0.5}},
          "animations": [
            {{"clip": "walk", "property": "rotation_euler", "axis": "z",
              "keyframes": [[1,0.0],[15,-0.03],[30,0.0],[45,0.03],[60,0.0]], "loop": true}}
          ]
        }},
        {{
          "name": "joint_neck", "body_part": "neck", "deforms_mesh": false,
          "position_normalized": {{"x": 0.5, "y": 0.70, "z": 0.5}},
          "animations": [
            {{"clip": "walk", "property": "rotation_euler", "axis": "z",
              "keyframes": [[1,0.0],[15,-0.02],[30,0.0],[45,0.02],[60,0.0]], "loop": true}}
          ]
        }},
        {{
          "name": "joint_head", "body_part": "head", "deforms_mesh": false,
        ...
        {{
          "name": "joint_shoulder_left", "body_part": "shoulder", "deforms_mesh": true,
          "position_normalized": {{"x": 0.15, "y": 0.50, "z": 0.5}},
          "animations": [
            {{"clip": "walk", "property": "rotation_euler", "axis": "x",
              "keyframes": [[1,0.0],[15,0.3],[30,0.0],[45,-0.3],[60,0.0]], "loop": true}}
          ]
        }},
        {{
          "name": "joint_shoulder_right", "body_part": "shoulder", "deforms_mesh": true,
        ...
        {{
          "name": "joint_elbow_left", "body_part": "elbow", "deforms_mesh": true,
          "position_normalized": {{"x": 0.08, "y": 0.42, "z": 0.5}},
          "animations": [
            {{"clip": "walk", "property": "rotation_euler", "axis": "x",
              "keyframes": [[1,0.0],[15,0.2],[30,0.4],[45,0.2],[60,0.0]], "loop": true}}
          ]
        }},
        {{
          "name": "joint_elbow_right", "body_part": "elbow", "deforms_mesh": true,
        ...
        {{
          "name": "joint_hand_left", "body_part": "hand", "deforms_mesh": true,
          "position_normalized": {{"x": 0.0, "y": 0.35, "z": 0.5}},
          "animations": [
            {{"clip": "walk", "property": "rotation_euler", "axis": "x",
              "keyframes": [[1,0.0],[15,0.1],[30,0.2],[45,0.1],[60,0.0]], "loop": true}}
          ]
        }},
        {{
          "name": "joint_hand_right", "body_part": "hand", "deforms_mesh": true,
        ...
        {{
          "name": "joint_hip_left", "body_part": "hip", "deforms_mesh": true,
          "position_normalized": {{"x": 0.38, "y": 0.22, "z": 0.5}},
          "animations": [
            {{"clip": "walk", "property": "rotation_euler", "axis": "x",
              "keyframes": [[1,0.0],[15,-0.4],[30,0.0],[45,0.4],[60,0.0]], "loop": true}}
          ]
        }},
        {{
          "name": "joint_hip_right", "body_part": "hip", "deforms_mesh": true,
        ...
        {{
          "name": "joint_knee_left", "body_part": "leg", "deforms_mesh": true,
        ...
        {{
          "name": "joint_knee_right", "body_part": "leg", "deforms_mesh": true,
        ...
        {{
          "name": "joint_foot_left", "body_part": "foot", "deforms_mesh": true,
          "position_normalized": {{"x": 0.35, "y": 0.0, "z": 0.5}},
          "animations": [
            {{"clip": "walk", "property": "rotation_euler", "axis": "x",
              "keyframes": [[1,0.0],[15,-0.2],[30,-0.1],[45,0.1],[60,0.0]], "loop": true}}
          ]
        }},
        {{
          "name": "joint_foot_right", "body_part": "foot", "deforms_mesh": true,
...
YOUR TASK:
    ...
    • For each joint, position_normalized should reflect the ACTUAL ATTACHMENT POINT location, not the joint_hints
    • ENSURE that Shoulders MUST be at the visual junction where the arms/elbows branch FROM the body, not the head
    • neck: where head visibly connects to body
    • hips: where legs branch from body
    • elbows/legs: extend from attachment points toward extremities
    • hands/feet: at the tips/extremities
    - Keep the animations exactly as shown — Gemini must NOT change keyframe values
    - Adjust joint positions to match where limbs actually are in the image
    - Left/right sides must be symmetric
    - If the object has no arms (e.g. snake, fish), remove shoulder/elbow/hand joints
    ...
    • Chest: y≈0.55
    • Shoulders: x≈0.2 (left), x≈0.8 (right), y≈0.40-0.45 ← LOWER than chest
    • elbows: x≈0.1 (left), x≈0.9 (right), y≈0.40-0.45
    • Hands: x=0.0 (left), x=1.0 (right), y≈0.35
    
    DO NOT cluster joints near center (x=0.5)!
    Spread them across the FULL WIDTH of the model.
    Left side should be clearly left (x < 0.5)
    Right side should be clearly right (x > 0.5)
    Extremities at actual mesh edges (x=0.0 or x=1.0)
    
    CRITICAL:
    - a t-pose or a-pose for easy rigging
    • Neck: junction where head/floret attaches to body, y around 0.70
    • Head: above neck, extends to top (y=0.85-1.0)
    • Pelvis: y around 0.20-0.25, parent of all legs
    • Shoulders: children of CHEST, not spine or pelvis
    • elbows: shoulder→elbow→hand chains, extending outward
    • Legs: pelvis→leg→foot chains, extending downward
    • All joint names UNIQUE
    • If left side has N joints, right side must have N joints
    • Complete chains for BOTH sides: pelvis → leg_left → foot_left AND pelvis → leg_right → foot_right
    • Return all joint_hints and skeleton bones for both sides
    • Joint Hierarchy vs Physical Position

    SKELETON HIERARCHY (parent-child relationships):
      chest → shoulder_left → elbow_left → hand_left

    BUT PHYSICAL POSITIONING is different:
      • The shoulder_left JOINT must be positioned at the OUTER EDGE of the mesh
        where the mesh physically branches OUT to the elbow, NOT at the chest center, and not within the mesh
      

    EXAMPLE:
      ✗ WRONG: shoulder_left positioned at chest center (x=0.5, y=0.5)
      ✓ CORRECT: shoulder_left positioned at left elbow branch where mesh visibly branches from body (x=0.15, y=0.45)
                 AND skeleton shows: chest → shoulder_left

    For  character:
      • Left shoulder attaches at the LEFT OUTER EDGE of the mesh
      • Right shoulder attaches at the RIGHT OUTER EDGE of the mesh
      • Position them OUTSIDE/AT the mesh boundary, not inside the body center
    """
